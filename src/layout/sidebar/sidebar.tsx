...
const NavLink: React.FC<NavLinkProps> = ({ title, link, dropdown, icon }) => {
  const location = useLocation();

  return (
    <Link to={link ? link : "#"}>
      <div className={`nav-link ${location.pathname === link ? "active" : ""}`}>
...
const Sidebar: React.FC = () => {
  ...
  return (
    <div className="sidebar">
      ...
      <NavLink icon={<HomeIcon />} title="Dashboard" link="/dashboard" />
      ...
      <div onClick={signOut}>
        <NavLink icon={<LogoutIcon />} title="Logout" />
      </div>
    </div>
  );
};
